...
"""
def get_data():
   data=bus.read_i2c_block_data(arduinoAddress, 0,5);
   #print(data)
   return data;
...
def get_float(data, index):
   ...
   checksum_rx = data[4]
   checksum_calc = bytes1[0] ^ bytes1[1] ^ bytes1[2] ^ bytes1[3]
   
   chks = checksum_rx == checksum_calc
   #print (bytes1)
   return (float(struct.unpack("<f", bytes(bytes1))[0]),chks)
